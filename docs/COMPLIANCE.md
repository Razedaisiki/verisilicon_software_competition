...
| Baseline versus quality selection | Provisional | Public CLI paths select `BicubicBaseline`; quality variants and `RecommendedBaselineV1` remain explicit development paths. Eval30 selected the ungated parameter candidate above the local bicubic anchor in both metrics, while the recommended and confidence-gated experiments were rejected. No CLI switch is inferred from local data. | Official baseline definition and evidence that any quality candidate meets the admission comparison. |
