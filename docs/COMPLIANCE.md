...
| Baseline versus quality selection | Provisional | Public CLI paths select `BicubicBaseline`; `QualityPipeline` and `RecommendedBaselineV1` remain explicit development paths. Eval30 measured the recommended experiment below bicubic, so no default switch was made. | Official baseline definition and evidence that any quality candidate meets the admission comparison. |
