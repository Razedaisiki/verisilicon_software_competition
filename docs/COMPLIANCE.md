...
| Baseline versus quality selection | Provisional | Public CLI paths still select the ungated `SelectedQualityPipeline` with parameters 64/2/32/64 pending a separate promotion commit. `BicubicBaseline`, frozen `QualityPipeline`, `RecommendedBaselineV1`, confidence-gated, bilinear-chroma, and fine-finalist 64/2/24/80 remain explicit comparison paths. Eval30 selected the fine finalist for both metrics in every fold. The one-candidate 100-image DIV2K validation comparison improved over the previous selection by +0.051761 dB Y-PSNR and +0.000836439 Y-SSIM, with both metrics positive on every image. The recommended, confidence-gated, and bilinear-chroma experiments were rejected. | Official baseline definition and evidence that the selected candidate meets the admission comparison. |
