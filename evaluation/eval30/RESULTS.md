...
The bounded 32-candidate fine sweep used the then-selected `64/2/32/64`
configuration as its comparison anchor. That historical anchor remains frozen
after the later public promotion. All five training folds independently
...
run on the separate 100-pair DIV2K validation set. This recorded sweep did not
itself change the public pipeline; the later accepted validation did.
...
The fixed-point gate was compared against both bicubic and the then-selected
`64/2/32/64` ungated control with explicit evaluator selectors. Both paths are
frozen for report reproduction.
...
The isolated candidate kept the then-selected `64/2/32/64`
bicubic-plus-enhanced luma and changed only Cb/Cr scaling to the project
bilinear 2x implementation. Both paths are frozen for report reproduction.
